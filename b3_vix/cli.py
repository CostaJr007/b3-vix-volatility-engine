...
def main():
    ...
    p_vix.add_argument("--near-du", type=int, default=15, help="Near-term expiration in business days")
    p_vix.add_argument("--next-du", type=int, default=35, help="Next-term expiration in business days")
    ...
    if args.command == "greeks":
        ...
        near_chain = B3MarketData.generate_bova11_chain(args.spot, du_to_expiry=args.near_du, risk_free_rate=args.rate)
        next_chain = B3MarketData.generate_bova11_chain(args.spot, du_to_expiry=args.next_du, risk_free_rate=args.rate)
        r_cont = DICurveInterpolator.from_dict({21: args.rate}).continuous_rate(21)

        v1 = VixBovaEngine.compute_single_term_variance(near_chain, time_to_exp=args.near_du / 252.0, r=r_cont, business_days=args.near_du)
        v2 = VixBovaEngine.compute_single_term_variance(next_chain, time_to_exp=args.next_du / 252.0, r=r_cont, business_days=args.next_du)
        ...
        print(f"Near-Term Variance:  {v1.variance:.6f} ({args.near_du} DU)")
        print(f"Next-Term Variance:  {v2.variance:.6f} ({args.next_du} DU)")
